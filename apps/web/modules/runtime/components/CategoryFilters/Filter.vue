<template>
  <SfAccordionItem v-if="facet" v-model="open">
    <template #summary>
      <div class="flex justify-between md:justify-start border-b-2 border-black md:border-0 items-center p-2 mb-0 select-none" @mouseover="open = true" @mouseleave="open = false">
        <p class="font-medium typography-headline-5 flex-1 md:flex-0">{{ facetGetters.getName(facet) }}</p>
        <SfIconChevronLeft size="lg" :class="['hidden md:block text-neutral-500', open ? 'rotate-90' : '-rotate-90']" />
        <SfIconAdd v-if="!open" class="md:hidden" />
        <SfIconRemove v-if="open" class="md:hidden" />
      </div>
    </template>
    ...
    <div v-else class="md:mb-4 py-2 flex flex-col md:absolute z-[1001] bg-[#f7f7f7]" @mouseover="open = true" @mouseleave="open = false">
      <SfListItem
        ...
        class="px-1.5 py-2 bg-transparent hover:bg-transparent !w-auto z-[100]"
      >
        <template #prefix>
          <SfCheckbox
            :id="filter.id"
